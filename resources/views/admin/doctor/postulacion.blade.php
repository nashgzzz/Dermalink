                ...
                @switch($applyDetail->type)
                    @case(0)
                        ...
                        @if ($applyDetail->relation != null)
                                @if ($applyDetail->relation->estado == 1)
                                    ...
                                    @endif
                                    @if ($answer == null)
                                        <div class="col-md-4 mb-4 invisible section-{{$applyDetail->relation->apply_detail_option->apply_detail->id}}" id="hidden-{{$applyDetail->relation->apply_detail_option_id}}" name="{{$applyDetail->relation->estado}}">
                                        <small>{{$applyDetail->question}}</small><br>
                                            <input class="form-control" {{ $bloqueo == true ? 'readonly' : '' }} placeholder="{{$applyDetail->placeholder}}" type="text" {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} name="{{ $applyDetail->id }}" value="">
                                        </div>
                                    @else
                                        <div class="col-md-4 mb-4 section-{{$applyDetail->relation->apply_detail_option->apply_detail->id}}" id="hidden-{{$applyDetail->relation->apply_detail_option_id}}" name="{{$applyDetail->relation->estado}}">
                                        <small>{{$applyDetail->question}}</small><br>
                                            <input class="form-control" {{ $bloqueo == true ? 'readonly' : '' }} placeholder="{{$applyDetail->placeholder}}" type="text" {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} name="{{ $applyDetail->id }}" value="{{ $answer }}">
                                        </div>
                                    @endif
                                @else
                                    <div class="col-md-4 mb-4 section-{{$applyDetail->relation->apply_detail_option->apply_detail->id}}" id="hidden-{{$applyDetail->relation->apply_detail_option_id}}" name="{{$applyDetail->relation->estado}}">
                                        <small>{{$applyDetail->question}}</small><br>
                                        <input class="form-control" {{ $bloqueo == true ? 'readonly' : '' }} placeholder="{{$applyDetail->placeholder}}" type="text" {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} name="{{ $applyDetail->id }}" value="{{ $answer == '' ? '' : $answer }}">
                                    </div>
                                @endif
                            @else

                                <div class="col-md-4 mb-4 {{$visible}}">
                                    <small>{{$applyDetail->question}}-3</small><br>
                                    <input class="form-control" {{ $bloqueo == true ? 'readonly' : '' }} placeholder="{{$applyDetail->placeholder}}" type="text" {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} name="{{ $applyDetail->id }}" value="{{ $answer == '' ? '' : $answer }}">
                                </div>
                        @endif
                    ...
                    @case(2)
                                ...
                                @if ($applyDetail->relation != null)
                                    <h1>no null</h1>
                                    @php
                                        $answerId =$applyDetail->relation->apply_detail_option->id;
                                    @endphp
                                @else
                                    <div class="col-md-4 mb-4">
                                        <small>{{$applyDetail->question}}</small><br>
                                ...
                                @if (!$bloqueo == true)
                                    ...
                                    @foreach ($applyDetail->apply_detail_options as $applyOption)
                                        @empty ( $applyOption->reation)
                                            <input type="hidden" name="{{$applyOption->id}}" value="{{$applyOption->id}}"/>
                                        @endempty
                                    @endforeach
                                @else
                                    <input {{ $bloqueo == true ? 'readonly' : '' }} class="form-control" type="text"   {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} placeholder="{{$applyDetail->placeholder}}" name="{{  $applyDetail->id }}" value="{{ $answer == null ? '' : $answer }}">
                    ...
                    @if ($applyDetail->relation != null)
                            ...
                            <div class="col-md-4 mb-4  {{$visible}} section-{{$applyDetail->relation->apply_detail_option->apply_detail->id}}" id="hidden-{{$applyDetail->relation->apply_detail_option_id}}" name="{{$applyDetail->relation->estado}}">
                                <small>{{$applyDetail->question}}</small><br>
                                 @if (!$bloqueo == true && !$answer == null)
                                    <input {{ $bloqueo == true ? 'readonly' : '' }} class="form-control" type="file" placeholder="{{$applyDetail->placeholder}}" name="{{  $applyDetail->id }}" value="{{ $answer == null ? '' : $answer }}">
                                @else
                                    <input {{ $bloqueo == true ? 'readonly' : '' }} class="form-control" type="file"   {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} placeholder="{{$applyDetail->placeholder}}" name="{{  $applyDetail->id }}" value="{{ $answer == null ? '' : $answer }}">
                                @endif
                                @if (!$answer == null)
                                    <div>
                                        <br>
                        ...
                        @else
                            ...
                            @if ($answer == null)
                                <div class="col-md-4 mb-4 {{$visible}} section-{{$applyDetail->relation->apply_detail_option->apply_detail->id}}" id="hidden-{{$applyDetail->relation->apply_detail_option_id}}" name="{{$applyDetail->relation->estado}}">
                                    <small>{{$applyDetail->question}} - 1</small><br>
                                    @if (!$bloqueo == true)
                                    <input {{ $bloqueo == true ? 'readonly' : '' }} class="form-control" type="file" placeholder="{{$applyDetail->placeholder}}" name="{{  $applyDetail->id }}">
                                    @else
                                        <input {{ $bloqueo == true ? 'readonly' : '' }} class="form-control" type="file"   {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} placeholder="{{$applyDetail->placeholder}}" name="{{  $applyDetail->id }}" >
                                    @endif
                                </div>
                            @else
                                @if (!$answer == null)
                                    <div class="col-md-4 mb-4 {{$visible}} section-{{$applyDetail->relation->apply_detail_option->apply_detail->id}}" id="hidden-{{$applyDetail->relation->apply_detail_option_id}}" name="{{$applyDetail->relation->estado}}">
                                        <small>{{$applyDetail->question}} - 2</small><br>
                                        <input {{ $bloqueo == true ? 'readonly' : '' }} class="form-control" type="file"   {{ $applyDetail->obligatorio == 1 ? 'required' : '' }} placeholder="{{$applyDetail->placeholder}}" name="{{  $applyDetail->id }}">
                                        <div>
                                            <br>
                                            <a class="bg-exito px-2 py-1 rounded text-sm text-white" href="{{ asset($answer)}}" target="_blank">Ver archivo</a>
                    ...
                    @case(7)
                        ...
                        <div class="col-md-4 mb-4">
                            ...
                            <select
                                ...
                                <option value="{{$answerId}}">{{ $answer == null ? '' : $answer }}</option>
                                <option>---Selecciona---</option>
                                @foreach ($applyDetail->apply_detail_options as $applyOption)
